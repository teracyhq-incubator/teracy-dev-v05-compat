module TeracyDev
  module Helpers
    ...
    def db_tbl_exists?(host, username, pwd, db_name, tbl)
      query_string = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '#{tbl}'"
      ::Mysql.new(host, username, pwd).select_db(db_name).query(query_string).num_rows > 0
    end
    ...
    def db_column_exists?(host, username, pwd, db_name, tbl, colum)
      query_string = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '#{tbl}' and COLUMN_NAME = '#{colum}'"
      ::Mysql.new(host, username, pwd).select_db(db_name).query(query_string).num_rows > 0
    end
